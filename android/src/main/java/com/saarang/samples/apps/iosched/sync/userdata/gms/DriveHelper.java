 ...
 */
package com.saarang.samples.apps.iosched.sync.userdata.gms;
...
import com.saarang.samples.apps.iosched.sync.userdata.util.UserDataHelper;
...
public class DriveHelper {
    ...
    private static final String TAG = LogUtils.makeLogTag(DriveHelper.class);
    ...
    static void saveToDrive(DriveFile file, Set<String> contents,
        ...
        byte[] serializedContents = UserDataHelper.toByteArray(contents);
    ...
    static void createNewDriveFile(Set<String> contents,
        ...
        os.write(UserDataHelper.toByteArray(contents));
        ...
        Log.d(TAG, "Content saved to new Drive file: "+new String(UserDataHelper.toByteArray(contents),
                Charsets.UTF_8));
    ...
    static public Set<String> loadFromCloud(DriveFile file, GoogleApiClient apiClient)
        ...
        try {
            FileInputStream is = new FileInputStream(contentsResult.getContents()
                    .getParcelFileDescriptor().getFileDescriptor());
            String contents = UserDataHelper.fromStreamToString(is);
            file.discardContents(apiClient, contentsResult.getContents());

            LogUtils.LOGD(TAG, "Contents in the cloud file: [" + contents + "]");

            return UserDataHelper.fromString(contents);
