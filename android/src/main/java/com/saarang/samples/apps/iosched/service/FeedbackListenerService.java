...
public class FeedbackListenerService extends WearableListenerService
    ...
    private static final String TAG = LogUtils.makeLogTag(FeedbackListenerService.class);
        ...
        for (DataEvent event : dataEvents) {
            ...
            if (event.getType() == DataEvent.TYPE_CHANGED) {
                if (PATH_RESPONSE.equals(path)) {
                    ...
                    LogUtils.LOGD(TAG, "jsonString is: " + jsonString);
    ...
    private void saveFeedback(String jsonString) {
        try {
            ...
            if (null != jsonArray) {
                ...
                LogUtils.LOGD(TAG, "Feedback answers received from the wear: " + result.toString());
                FeedbackUtils.saveSessionFeedback(this, sessionId, answers[0], answers[1],
                        answers[2], answers[3], null);
            }

        } catch (JSONException e) {
            LogUtils.LOGE(TAG, "Failed to parse the json received from the wear", e);
        }
    }
