...
public class CollectionView extends ListView {
    ...
    public CollectionView(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (attrs != null) {
            final TypedArray xmlArgs = context.obtainStyledAttributes(attrs,
                    com.saarang.samples.apps.iosched.R.styleable.CollectionView, defStyle, 0);
            mInternalPadding = xmlArgs.getDimensionPixelSize(
                    com.saarang.samples.apps.iosched.R.styleable.CollectionView_internalPadding, 0);
            mContentTopClearance = xmlArgs.getDimensionPixelSize(
                    com.saarang.samples.apps.iosched.R.styleable.CollectionView_contentTopClearance, 0);
        }
    }
    ...
    public void updateInventory(final Inventory inv, boolean animate) {
        if (animate) {
            LogUtils.LOGD(TAG, "CollectionView updating inventory with animation.");
            setAlpha(0);
            updateInventoryImmediate(inv, animate);
            doFadeInAnimation();
        } else {
            LogUtils.LOGD(TAG, "CollectionView updating inventory without animation.");
            updateInventoryImmediate(inv, animate);
        }
    }
    ...
    private LinearLayout.LayoutParams setupLayoutParams(View view) {
        ...
        if (view.getLayoutParams() instanceof LinearLayout.LayoutParams) {
            ...
            LogUtils.LOGW(TAG, "Unexpected class for collection view item's layout params: " +
