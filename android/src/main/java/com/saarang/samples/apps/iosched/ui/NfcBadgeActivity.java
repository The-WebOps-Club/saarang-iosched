...
public class NfcBadgeActivity extends Activity {
    private static final String TAG = LogUtils.makeLogTag(NfcBadgeActivity.class);
    private static final String ATTENDEE_URL_PREFIX = "\u0004plus.google.com/";

    // For debug purposes
    public static final String ACTION_SIMULATE = "com.saarang.samples.apps.iosched.ACTION_SIMULATE";
    ...
    private void recordBadge(String url) {
        LogUtils.LOGD(TAG, "Recording badge, URL " + url);
        if (url.startsWith(ATTENDEE_URL_PREFIX)) {
            ...
            LogUtils.LOGD(TAG, "URL in badge (" + url + ") does not start with prefix URL ("
                    + ATTENDEE_URL_PREFIX + ")");
            LogUtils.LOGW(TAG, "Invalid URL in badge: " + url);
        }
    }
    ...
    private void addToHistory(String url) {
        ...
        if (!isValidPlusId(plusId)) {
            LogUtils.LOGE(TAG, "Unknown ID format: " + plusId);
            return;
        }
        if (isDummyPlusId(plusId)) {
            Toast.makeText(this, com.saarang.samples.apps.iosched.R.string.people_ive_met_dummy_badge, Toast.LENGTH_SHORT).show();
            return;
        }
