 ...
 */
package com.saarang.samples.apps.iosched.gcm.command;
...
import com.saarang.samples.apps.iosched.gcm.GCMCommand;
...
public class NotificationCommand extends GCMCommand {
    ...
    public void execute(Context context, String type, String payload) {
        LogUtils.LOGI(TAG, "Received GCM message: " + type);
        LogUtils.LOGI(TAG, "Parsing GCM notification command: " + payload);
        Gson gson = new Gson();
        ...
        try {
            ...
            ex.printStackTrace();
            LogUtils.LOGE(TAG, "Failed to parse GCM notification command.");
            return;
        }
    ...
    private void processCommand(Context context, NotificationCommandModel command) {
        ...
        final String title = TextUtils.isEmpty(command.title) ?
                context.getString(com.saarang.samples.apps.iosched.R.string.app_name) : command.title;
