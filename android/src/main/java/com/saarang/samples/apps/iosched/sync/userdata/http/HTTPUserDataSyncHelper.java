...
package com.saarang.samples.apps.iosched.sync.userdata.http;
...
import com.saarang.samples.apps.iosched.sync.userdata.UserAction;
...
public class HTTPUserDataSyncHelper extends AbstractUserDataSyncHelper {
    ...
    private String extractGcmKey(Set<String> remote) {
        ...
        for (String s : remote) {
            if (s.startsWith(GCM_KEY_PREFIX)) {
                ...
                LogUtils.LOGD(TAG, "Remote data came with GCM key: "
    ...
    protected boolean syncImpl(List<UserAction> actions, boolean hasPendingLocalData) {
        try {
            LogUtils.LOGD(TAG, "Now syncing user data.");
    ...
    private static final String TAG = LogUtils.makeLogTag(HTTPUserDataSyncHelper.class);
