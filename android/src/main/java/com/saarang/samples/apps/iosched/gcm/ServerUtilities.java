...
public final class ServerUtilities {
    private static final String TAG = LogUtils.makeLogTag("GCMs");

    private static final String PREFERENCES = "com.saarang.samples.apps.iosched.gcm";
    ...
    private static boolean checkGcmEnabled() {
        if (TextUtils.isEmpty(Config.GCM_SERVER_URL)) {
            LogUtils.LOGD(TAG, "GCM feature disabled (no URL configured)");
            return false;
        } else if (TextUtils.isEmpty(Config.GCM_API_KEY)) {
            LogUtils.LOGD(TAG, "GCM feature disabled (no API key configured)");
            return false;
        } else if (TextUtils.isEmpty(Config.GCM_SENDER_ID)) {
            LogUtils.LOGD(TAG, "GCM feature disabled (no sender ID configured)");
            return false;
        }
        return true;
    }
    ...
    public static boolean register(final Context context, final String gcmId, final String gcmKey) {
        ...
        LogUtils.LOGI(TAG, "registering device (gcm_id = " + gcmId + ")");
        String serverUrl = Config.GCM_SERVER_URL + "/register";
        LogUtils.LOGI(TAG, "registering on GCM with GCM key: " + AccountUtils.sanitizeGcmKey(gcmKey));
    ...
    static void unregister(final Context context, final String gcmId) {
        ...
        LogUtils.LOGI(TAG, "unregistering device (gcmId = " + gcmId + ")");
    ...
    public static void notifyUserDataChanged(final Context context) {
        ...
        LogUtils.LOGI(TAG, "Notifying GCM that user data changed");
        ...
        try {
            ...
            LogUtils.LOGE(TAG, "Unable to notify GCM about user data change", e);
        }
    }
    ...
    private static void setRegisteredOnServer(Context context, boolean flag, String gcmId, String gcmKey) {
        ...
        LogUtils.LOGD(TAG, "Setting registered on server status as: " + flag + ", gcmKey="
    ...
    public static boolean isRegisteredOnServer(Context context, String gcmKey) {
        ...
        if (regTS > yesterdayTS) {
            ...
            if (registeredGcmKey.equals(gcmKey)) {
                LogUtils.LOGD(TAG, "GCM registration is valid and for the correct gcm key: "
                        + AccountUtils.sanitizeGcmKey(registeredGcmKey));
                return true;
            }
            LogUtils.LOGD(TAG, "GCM registration is for DIFFERENT gcm key "
    ...
    private static void post(String endpoint, Map<String, String> params, String key)
        ...
        LogUtils.LOGV(TAG, "Posting '" + body + "' to " + url);
