...
public class RecentTasksStyler {
    ...
    public static void styleRecentTasksEntry(Activity activity) {
        ...
        int colorPrimary = resources.getColor(com.saarang.samples.apps.iosched.R.color.theme_primary);

        if (sIcon == null) {
            // Cache to avoid decoding the same bitmap on every Activity change
            sIcon = BitmapFactory.decodeResource(resources, com.saarang.samples.apps.iosched.R.drawable.ic_stat_notification);
        }
