...
public class LoginAndAuthHelper implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, ResultCallback<People.LoadPeopleResult> {
    ...
    public LoginAndAuthHelper(Activity activity, Callbacks callbacks, String accountName) {
        LogUtils.LOGD(TAG, "Helper created. Account: " + mAccountName);
    ...
    public void retryAuthByUserRequest() {
        LogUtils.LOGD(TAG, "Retrying sign-in/auth (user-initiated).");
        if (!mGoogleApiClient.isConnected()) {
            ...
            LogUtils.LOGD(TAG, "No need to retry auth: GoogleApiClient is connected and we have auth token.");
        }
    }
    ...
    public void start() {
        ...
        if (mResolving) {
            ...
            LogUtils.LOGD(TAG, "Helper ignoring signal to start because we're resolving a failure.");
            return;
        }
        LogUtils.LOGD(TAG, "Helper starting. Connecting " + mAccountName);
        if (mGoogleApiClient == null) {
            LogUtils.LOGD(TAG, "Creating client.");
        ...
        LogUtils.LOGD(TAG, "Connecting client.");
        mGoogleApiClient.connect();
    }
    ...
    public void onConnectionSuspended(int i) {
        LogUtils.LOGD(TAG, "onConnectionSuspended.");
    }
    ...
    public void stop() {
        ...
        if (mTokenTask != null) {
            LogUtils.LOGD(TAG, "Helper cancelling token task.");
            mTokenTask.cancel(false);
        }
        mStarted = false;
        if (mGoogleApiClient.isConnected()) {
            LogUtils.LOGD(TAG, "Helper disconnecting client.");
    ...
    public void onConnectionFailed(ConnectionResult connectionResult) {
        ...
        if (connectionResult.hasResolution()) {
            if (sCanShowSignInUi) {
                LogUtils.LOGD(TAG, "onConnectionFailed, with resolution. Attempting to resolve.");
                ...
                try {
                    ...
                    LogUtils.LOGE(TAG, "SendIntentException occurred: " + e.getMessage());
                    e.printStackTrace();
                }
            } else {
                LogUtils.LOGD(TAG, "onConnectionFailed with resolution but sCanShowSignInUi==false.");
                reportAuthFailure();
            }
            return;
        }

        LogUtils.LOGD(TAG, "onConnectionFailed, no resolution.");
    ...
    public boolean onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_AUTHENTICATE ||
            ...
            LogUtils.LOGD(TAG, "onActivityResult, req=" + requestCode + ", result=" + resultCode);
            ...
            if (resultCode == Activity.RESULT_OK) {
                if (mGoogleApiClient != null) {
                    LogUtils.LOGD(TAG, "Since activity result was RESULT_OK, reconnecting client.");
                    mGoogleApiClient.connect();
                } else {
                    LogUtils.LOGD(TAG, "Activity result was RESULT_OK, but we have no client to reconnect.");
                }
            } else if (resultCode == Activity.RESULT_CANCELED) {
                LogUtils.LOGD(TAG, "User explicitly cancelled sign-in/auth flow.");
                // save this as a preference so we don't annoy the user again
                PrefUtils.markUserRefusedSignIn(mAppContext);
            } else {
                LogUtils.LOGW(TAG, "Failed to recover from a login/auth failure, resultCode=" + resultCode);
            }
            return true;
        }
        return false;
    ...
    private void showAuthRecoveryFlow(Intent intent) {
        ...
        if (sCanShowAuthUi) {
            sCanShowAuthUi = false;
            LogUtils.LOGD(TAG, "Starting auth recovery Intent.");
            activity.startActivityForResult(intent, REQUEST_RECOVER_FROM_AUTH_ERROR);
        } else {
            LogUtils.LOGD(TAG, "Not showing auth recovery flow because sCanShowSignInUi==false.");
            reportAuthFailure();
        }
    }

    private void reportAuthSuccess(boolean newlyAuthenticated) {
        LogUtils.LOGD(TAG, "Auth success for account " + mAccountName + ", newlyAuthenticated=" + newlyAuthenticated);
        Callbacks callbacks;
        if (null != (callbacks = mCallbacksRef.get())) {
    ...
    private void reportAuthFailure() {
        LogUtils.LOGD(TAG, "Auth FAILURE for account " + mAccountName);
        Callbacks callbacks;
        if (null != (callbacks = mCallbacksRef.get())) {
    ...
    private class GetTokenTask extends AsyncTask<Void, Void, String> {
        ...
        protected String doInBackground(Void... params) {
            try {
                if (isCancelled()) {
                    LogUtils.LOGD(TAG, "doInBackground: task cancelled, so giving up on auth.");
                    return null;
                }

                LogUtils.LOGD(TAG, "Starting background auth for " + mAccountName);
                final String token = GoogleAuthUtil.getToken(mAppContext, mAccountName, AUTH_TOKEN_TYPE);

                // Save auth token.
                LogUtils.LOGD(TAG, "Saving token: " + (token == null ? "(null)" : "(length " +
                ...
                LogUtils.LOGE(TAG, "IOException encountered: " + e.getMessage());
            } catch (GoogleAuthException e) {
                LogUtils.LOGE(TAG, "GoogleAuthException encountered: " + e.getMessage());
            } catch (RuntimeException e) {
                LogUtils.LOGE(TAG, "RuntimeException encountered: " + e.getMessage());
            }
            return null;
        }
        ...
        protected void onPostExecute(String token) {
            ...
            if (isCancelled()) {
                LogUtils.LOGD(TAG, "Task cancelled, so not reporting auth success.");
            } else if (!mStarted) {
                LogUtils.LOGD(TAG, "Activity not started, so not reporting auth success.");
            } else {
                LogUtils.LOGD(TAG, "GetTokenTask reporting auth success.");
        ...
        private void postShowRecoveryDialog(final int statusCode) {
            ...
            if (isCancelled()) {
                LogUtils.LOGD(TAG, "Task cancelled, so not showing recovery dialog.");
                return;
            }

            LogUtils.LOGD(TAG, "Requesting display of recovery dialog for status code " + statusCode);
            activity.runOnUiThread(new Runnable() {
                ...
                public void run() {
                    if (mStarted) {
                        ...
                        LogUtils.LOGE(TAG, "Activity not started, so not showing recovery dialog.");
                    }
                }
            });
        }
        ...
        private void postShowAuthRecoveryFlow(final Intent intent) {
            ...
            if (isCancelled()) {
                LogUtils.LOGD(TAG, "Task cancelled, so not showing auth recovery flow.");
                return;
            }

            LogUtils.LOGD(TAG, "Requesting display of auth recovery flow.");
            activity.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    if (mStarted) {
                        ...
                        LogUtils.LOGE(TAG, "Activity not started, so not showing auth recovery flow.");
                    }
                }
            });
        }
